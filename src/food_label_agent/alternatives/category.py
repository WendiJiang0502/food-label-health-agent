...
CATEGORY_TERMS = {
    ...
    "instant_noodles": ("方便面", "即食面", "速食面", "杯面", "牛肉面"),
    "drink": ("饮料", "果汁", "汽水", "可乐", "植物蛋白饮品", "含乳饮料", "豆奶", "燕麦奶"),
    "dairy": ("乳制品", "纯牛奶", "牛奶", "酸奶", "发酵乳", "奶酪", "黄油", "稀奶油"),
    "snack": (
        "薯片", "锅巴", "虾条", "爆米花", "膨化食品", "坚果", "果仁", "瓜子",
        "开心果", "巴旦木",
    ),
    "confectionery": ("糖果", "巧克力", "奶糖", "奇趣蛋", "软糖", "硬糖", "果冻"),
    "prepared_meal": (
        "预制菜", "自热饭", "自热米饭", "即食米饭", "自热餐", "自热火锅", "便当"
    ),
    "frozen_food": (
        "速冻", "冷冻食品", "速冻水饺", "速冻汤圆", "冰淇淋", "冰激凌",
        "雪糕", "冰棒", "冰棍", "圣代", "雪糍", "花心筒", "甜品棒", "沁优杯",
        "雀巢8次方", "雀巢摩爵", "雀巢牛奶棒", "雀巢小杯",
    ),
...
# These terms describe storage/form rather than flavour. They must outrank
# embedded words such as “巧克力” or “牛奶” in an ice cream product name.
FROZEN_FORM_TERMS = frozenset(
    {
        "速冻",
        "冷冻食品",
        "速冻水饺",
...
def suggest_product_category(confirmed_fields: dict[str, str]) -> dict[str, Any]:
    ...
    frozen_form_match = any(term in product_name for term in FROZEN_FORM_TERMS)
    category = "frozen_food" if frozen_form_match else ranked[0]
    tied = (
        not frozen_form_match
        and len(ranked) > 1
        and scores[ranked[1]] == scores[category]
    )
