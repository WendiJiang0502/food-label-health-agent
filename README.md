...
官网商品页文字、官网页面截图和图片 URL 只用于证明转录来源，不能单独证明当前在售包装。替代品只有同时满足以下条件，才会标为 `fully_verified`：记录了真实 SKU/条码与规格；配料和营养背标由 JPEG、PNG 或 WebP 原图进入内容寻址存储；图片短边至少 480px、长边至少 640px，并通过最低清晰度与对比度检查；图片字节的 SHA-256 与记录及本地不可变文件一致；两名不同审核者完成复核；图片中的 SKU/规格与商品记录一致。一个同时清晰覆盖配料表和营养表的 `combined` 实物照片可以满足两类图片要求。只有快照元数据、但找不到对应原图或哈希不一致的记录，无法通过发现审核。

缺少上述证据的目录记录仍可在低风险场景作为“部分证据的同用途备选”，但界面必须显示实物背标未完成复核；严重过敏场景直接关闭。官方发现审核也不会把这类记录写入动态推荐目录。`/api/v1/alternatives/catalog-review-queue` 会列出待补 SKU、规格、配料图和营养图的商品。正式发布评测默认要求实物背标完整覆盖率为 100%，可用 `FOOD_LABEL_MIN_PACKAGING_SNAPSHOT_RATE` 调整，但不得借此绕过严重过敏安全门。

运营人员使用 `food-label-packaging-evidence capture` 把原图写入内容寻址存储并生成首次审核记录，使用 `review` 由第二名审核者核验原图哈希，最后使用 `attach` 将已验证快照绑定到同一 SKU/规格并刷新商品内容哈希。命令要求显式提供输入和输出文件，不会原地覆盖目录；审核后的商品 JSON 仍需通过官方发现审核门进入动态推荐目录。
