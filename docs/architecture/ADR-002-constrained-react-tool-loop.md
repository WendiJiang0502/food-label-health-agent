...
在 `evaluate_safety` 与 `final_safety_gate` 之间加入 `react_orchestrator`。它使用确定性、可测试的策略逐步选择下一项 MCP 调用，并且只允许以下工具：
...
系统第一次具备真实但有边界的工具选择能力，同时保持安全流程可复现、可审计和不依赖隐式思维链。当前选择策略仍是确定性的；未来即使引入模型提出候选动作，也必须通过相同的允许列表、前置条件、预算和最终安全门验证。
